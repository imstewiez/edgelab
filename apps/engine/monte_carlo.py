...
from quantlab_core import OUTPUTS_DIR, HORIZON, backtest, feature_path, session_mask, signals
...
def _simulate(R: np.ndarray, sims: int = 800, skip_prob: float = 0.03, slip_sigma: float = 0.035):
    ...
    for _ in range(sims):
        ...
        keep = rng.random(len(sample)) > skip_prob
        sample = sample[keep]
        if len(sample) == 0:
            sample = np.array([-1.0])
        shocks = rng.normal(loc=-0.012, scale=slip_sigma, size=len(sample))
        stressed = sample + shocks
    ...
    return {
        "median_totalR": round(float(np.median(totals)), 3),
        "p05_totalR": round(float(np.percentile(totals, 5)), 3),
        "p95_dd_R": round(float(np.percentile(dds, 95)), 3),
        "p99_dd_R": round(float(np.percentile(dds, 99)), 3),
        "p95_loss_streak": int(np.percentile(streaks, 95)),
        "profit_probability": round(float((totals > 0).mean()), 3),
        "ruin_probability": round(float((dds > 20).mean()), 3),
    }


def _candidate_key(row: dict) -> tuple:
    return (str(row.get("symbol", "")), str(row.get("tf", "")), str(row.get("concept", "")), str(row.get("session", "")), str(row.get("rr", "")), str(row.get("sl_mult", "")))
...
def _load_candidate_source(run_dir: Path) -> pd.DataFrame:
    ...
    stress = run_dir / "stage4_execution_stress.csv"
    if stress.exists():
        st = pd.read_csv(stress).replace([np.inf, -np.inf], np.nan).fillna("")
        st = st[st.stress_status.isin(["stress_pass", "stress_watchlist"])]
        if len(st):
            good = {_candidate_key(r)[:3] for r in st.to_dict("records")}
            df = df[df.apply(lambda x: (str(x.symbol), str(x.tf), str(x.concept)) in good, axis=1)]
    return df.head(80)
...
def _row_monte_carlo(row: dict):
    ...
    trades = backtest(df, b0 & sm, s0 & sm, rr, sl_mult, HORIZON.get(tf, 48))
    if trades.empty or len(trades) < 20:
        return None

    R = trades.R.astype(float).to_numpy()
    mc = _simulate(R)
    score = 0
    reasons = []
    if mc["profit_probability"] >= 0.80:
        score += 30
    else:
        reasons.append("profit probability too low")
    if mc["p05_totalR"] > 0:
        score += 25
    else:
        reasons.append("5th percentile total return is negative")
    if mc["p95_dd_R"] <= 12:
        score += 25
    elif mc["p95_dd_R"] <= 18:
        score += 12
    else:
        reasons.append("simulated drawdown too high")
    if mc["p95_loss_streak"] <= 8:
        score += 10
    else:
        reasons.append("loss-streak tail too high")
    if mc["ruin_probability"] <= 0.03:
        score += 10
    else:
        reasons.append("ruin probability too high")
    ...
    return {
        ...
        "mc_score": int(score),
        **mc,
        "verdict": "; ".join(reasons) if reasons else "Passed first Monte Carlo robustness gate",
        "ea_ready": False,
    }
...
def run_monte_carlo(scan_name: str | None = None, logger: Callable[[str], None] = print):
    ...
    passed = int((df.mc_status == "mc_pass").sum()) if not df.empty else 0
    watch = int((df.mc_status == "mc_watchlist").sum()) if not df.empty else 0
    failed = int((df.mc_status == "mc_fail").sum()) if not df.empty else 0
    ...
    summary = {
        ...
        "mc_fail": failed,
        "ea_ready": 0,
        "stage": "Stage 5 Monte Carlo robustness",
        "warning": "Monte Carlo uses historical trade R sequences from the current OHLC backtest. It is a robustness gate, not live-proof.",
