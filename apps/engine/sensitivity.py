...
from pipeline_io import EDGE_COLUMNS, MC_COLUMNS, SENSITIVITY_COLUMNS, STRESS_COLUMNS, VALIDATION_COLUMNS, WF_COLUMNS, safe_read_csv, safe_to_csv
...
def _variants(row: dict):
    lb = int(_num(row.get("lookback"), 20)); rr = _num(row.get("rr"), 1.0); sl = _num(row.get("sl_mult"), 1.0)
...
def _test_row(row: dict):
    symbol = str(row.get("symbol")); tf = str(row.get("tf")); concept = str(row.get("concept")); session = str(row.get("session", "all"))
    fp = feature_path(symbol, tf)
    if not fp.exists():
        return None
    df = pd.read_pickle(fp); mask = session_mask(df, session); rows = []
    ...
    pass_rate = float(variants.passed.mean()); median_pf = float(variants.pf.median()); min_pf = float(variants.pf.min()); median_exp = float(variants.expR.median()); max_dd = float(variants.maxDD_R.max())
    score = 0; reasons = []
    if pass_rate >= 0.55: score += 35
    elif pass_rate >= 0.35: score += 18
    else: reasons.append("parameter area is too narrow")
    if median_pf >= 1.25: score += 25
    elif median_pf >= 1.12: score += 12
    else: reasons.append("median PF is weak")
    if min_pf >= 0.95: score += 15
    else: reasons.append("nearby settings weaken")
    if median_exp > 0: score += 15
    else: reasons.append("median expectancy is not positive")
    if max_dd <= 16: score += 10
    else: reasons.append("drawdown expands")
...
def run_sensitivity(scan_name: str | None = None, logger: Callable[[str], None] = print):
    ...
    for i, row in enumerate(candidates.to_dict("records"), 1):
        ...
        if result:
            rows.append(result)
    df = pd.DataFrame(rows).sort_values("sensitivity_score", ascending=False) if rows else pd.DataFrame(columns=SENSITIVITY_COLUMNS)
    df = safe_to_csv(df, run_dir / "stage6_sensitivity.csv", SENSITIVITY_COLUMNS)
    summary = {"scan_name": run_dir.name, "validated_at": time.strftime("%Y-%m-%d %H:%M:%S"), "candidates_checked": int(len(df)), "sensitivity_pass": int((df.sensitivity_status == "sensitivity_pass").sum()) if not df.empty else 0, "sensitivity_watchlist": int((df.sensitivity_status == "sensitivity_watchlist").sum()) if not df.empty else 0, "sensitivity_fail": int((df.sensitivity_status == "sensitivity_fail").sum()) if not df.empty else 0, "ea_ready": 0, "stage": "Stage 6 parameter sensitivity", "warning": "A single winning parameter is not enough. This gate checks whether nearby parameters still work using exact setup_id handoffs.", "top": df.head(25).replace([np.inf, -np.inf], np.nan).fillna("").to_dict("records") if not df.empty else []}
