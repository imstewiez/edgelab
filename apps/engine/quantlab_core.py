...
def read_csv_flexible(path: Path) -> pd.DataFrame:
    ...
    for sep in [',',';','\t']:
        try:
            ...
            if 'date' in df.columns and 'time' in df.columns and 'open' in df.columns: df['time'] = df['date'].astype(str)+' '+df['time'].astype(str)
...
def score_health(symbol, tf, df, source):
    ...
    gaps = int((deltas > expected*3).sum()) if len(deltas) else 0; dup = int(d['time'].duplicated().sum()); days = max(1, (d['time'].max()-d['time'].min()).days)
    score = 100 - min(35,gaps*2) - min(20,dup)
    if len(d) < 1000 and tf != 'D1': score -= 25
    if days < 365: score -= 20
    score = max(0,int(score)); status = 'good' if score>=75 else ('usable' if score>=55 else 'weak')
    return {'symbol':symbol,'tf':tf,'source':source,'rows':int(len(d)),'start':str(d.time.min()),'end':str(d.time.max()),'coverage_days':int(days),'gap_count':gaps,'duplicate_count':dup,'quality_score':score,'status':status,'notes':'Short intraday history; avoid over-trusting results.' if days<365 and tf in ['M1','M5','M15'] else ''}
...
def signals(df,concept,lb):
    c=df.close; hi=df.high.rolling(lb).max().shift(1); lo=df.low.rolling(lb).min().shift(1); b=s=pd.Series(False,index=df.index)
...
def discover_edges(name,mode,symbols,tfs,logger):
    ...
    for _,r in cat.iterrows():
        ...
        concepts=['breakout_trend','breakout_fast','pullback_ema21','compression_breakout','sweep_reclaim','prev_day_sweep'] + (['asian_breakout'] if tf in ['M5','M15','M30','H1'] else [])
        sessions=['all','london_ny','ny','overlap']; lbs=[20,50] if tf in ['H1','H4','D1'] else [12,20,48]; min_tr=25 if tf=='D1' else 35
                ...
                for sess in sessions:
                    ...
                    for rr in [1.0,1.4,2.0,2.5]:
                        for slm in [1.0,1.4,2.0]:
                            ...
                            split=max(1,int(len(tr)*.7)); train=st(tr.iloc[:split]); test=st(tr.iloc[split:]) or {}; mon=tr.groupby(['year','month']).R.sum(); pos=float((mon>0).mean()) if len(mon) else 0
    ...
    return {'scan_name':name,'datasets':len(datasets),'tested_edges':len(res),'candidates':len(cand),'rejected':len(rej)}
