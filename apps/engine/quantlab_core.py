...
from broker_config import load_broker_profile, symbol_profile
...
HEALTH_PATH = STORE / "data_health.json"
LATEST_CARDS_PATH = OUTPUTS_DIR / "latest_edge_cards.json"
...
SETUP_FIELDS = ["symbol", "tf", "concept", "session", "lookback", "rr", "sl_mult"]
...
def setup_tuple(row: dict | pd.Series) -> tuple[str, ...]:
    return tuple(_fmt(row.get(k, "")) for k in SETUP_FIELDS)


def make_setup_id(symbol: str, tf: str, concept: str, session: str, lookback: Any, rr: Any, sl_mult: Any) -> str:
    raw = "|".join([_fmt(symbol).upper(), _fmt(tf).upper(), _fmt(concept), _fmt(session), _fmt(lookback), _fmt(rr), _fmt(sl_mult)])
    digest = hashlib.sha1(raw.encode("utf-8")).hexdigest()[:10]
    return f"{_fmt(symbol).upper()}_{_fmt(tf).upper()}_{_fmt(concept)}_{digest}"


def row_setup_id(row: dict | pd.Series) -> str:
    return make_setup_id(row.get("symbol", ""), row.get("tf", ""), row.get("concept", ""), row.get("session", ""), row.get("lookback", ""), row.get("rr", ""), row.get("sl_mult", ""))


def setup_key_set(df: pd.DataFrame) -> set[tuple[str, ...]]:
    if df.empty:
        return set()
    return {setup_tuple(r) for r in df.to_dict("records")}


def filter_by_setup_keys(source: pd.DataFrame, gate: pd.DataFrame) -> pd.DataFrame:
    if source.empty or gate.empty:
        return source.iloc[0:0]
    keys = setup_key_set(gate)
    return source[source.apply(lambda r: setup_tuple(r) in keys, axis=1)]
...
def read_csv_flexible(path: Path) -> pd.DataFrame:
    ...
    for sep in [",", ";", "\t"]:
        try:
            ...
            if {"time", "open", "high", "low", "close"}.issubset(df.columns):
                return df
...
def score_health(symbol, tf, df, source):
    ...
    expected = pd.Timedelta(minutes=TF_MINUTES.get(tf, 1))
...
def import_raw_data(logger: Callable[[str], None] = print):
    ...
    for i, f in enumerate(files, 1):
        ...
        keep = [c for c in ["time", "open", "high", "low", "close", "tick_volume", "spread_points", "real_volume", "spread"] if c in df.columns]
        ...
        rows.append({"symbol": sym, "tf": tf, "rows": int(len(df)), "start": str(df.time.min()), "end": str(df.time.max()), "source": str(f.relative_to(RAW_DIR)), "cache": str(cache_path(sym, tf).relative_to(STORE)), "has_spread_points": "spread_points" in df.columns})
...
def build_one_features(df, tf):
    ...
    df["atr_rank_500"] = df.atr14.rolling(500, min_periods=50).rank(pct=True); df["range_rank_200"] = df.range.rolling(200, min_periods=50).rank(pct=True); df["compression"] = (df.atr_rank_500 < 0.35) & (df.range_rank_200 < 0.45)
    df["trend_up"] = (df.close > df.ema200) & (df.ema21 > df.ema55); df["trend_down"] = (df.close < df.ema200) & (df.ema21 < df.ema55)
    df["impulse_up"] = (df.close > df.open) & (df.range > df.atr14 * 1.2) & (df.body_abs > df.range * 0.55); df["impulse_down"] = (df.close < df.open) & (df.range > df.atr14 * 1.2) & (df.body_abs > df.range * 0.55)
    df["bull_fvg"] = df.low > df.high.shift(2); df["bear_fvg"] = df.high < df.low.shift(2); df["bull_fvg_mid"] = ((df.low + df.high.shift(2)) / 2).where(df.bull_fvg); df["bear_fvg_mid"] = ((df.high + df.low.shift(2)) / 2).where(df.bear_fvg)
    df["bull_ob_high"] = df.open.shift(1).where(df.impulse_up & (df.close.shift(1) < df.open.shift(1))).ffill(limit=40); df["bull_ob_low"] = df.low.shift(1).where(df.impulse_up & (df.close.shift(1) < df.open.shift(1))).ffill(limit=40)
    df["bear_ob_low"] = df.open.shift(1).where(df.impulse_down & (df.close.shift(1) > df.open.shift(1))).ffill(limit=40); df["bear_ob_high"] = df.high.shift(1).where(df.impulse_down & (df.close.shift(1) > df.open.shift(1))).ffill(limit=40)
...
def signals(df, concept, lb):
    ...
    if concept == "breakout_trend": b = (c > hi) & df.trend_up & (df.ema21_slope_atr > 0.03); s = (c < lo) & df.trend_down & (df.ema21_slope_atr < -0.03)
    ...
    elif concept == "sweep_reclaim": b = (df.low < lo) & (c > lo) & (df.wick_pressure > 0.2); s = (df.high > hi) & (c < hi) & (df.wick_pressure < -0.2)
        ...
        b = eq_low & (df.low < lo) & (c > lo) & (df.lower_wick > df.upper_wick); s = eq_high & (df.high > hi) & (c < hi) & (df.upper_wick > df.lower_wick)
    elif concept == "bos_breakout": b = (c > hi) & (c.shift(1) <= hi) & (df.ema21 > df.ema55); s = (c < lo) & (c.shift(1) >= lo) & (df.ema21 < df.ema55)
    elif concept == "choch_reversal": b = df.trend_down.shift(1).fillna(False) & (c > hi) & (df.rsi14 > 50); s = df.trend_up.shift(1).fillna(False) & (c < lo) & (df.rsi14 < 50)
        ...
        b = df.trend_up & (df.low <= bull_mid) & (c > bull_mid) & (c > df.open); s = df.trend_down & (df.high >= bear_mid) & (c < bear_mid) & (c < df.open)
    elif concept == "order_block_retest": b = df.trend_up & (df.low <= df.bull_ob_high) & (c > df.bull_ob_high) & (df.bull_ob_high > df.bull_ob_low); s = df.trend_down & (df.high >= df.bear_ob_low) & (c < df.bear_ob_low) & (df.bear_ob_high > df.bear_ob_low)
    return b.fillna(False), s.fillna(False)


def _spread_points_at(df: pd.DataFrame, entry_idx: int, sym_profile: dict[str, Any]) -> tuple[float, str]:
    if "spread_points" in df.columns:
        try:
            v = float(df.spread_points.iloc[entry_idx])
            if np.isfinite(v) and v > 0: return v, "data"
        except Exception: pass
    return float(sym_profile.get("default_spread_points", 20)), "profile_default"


def estimate_cost_r(df: pd.DataFrame, entry_idx: int, symbol: str, risk: float, spread_mult: float = 1.0, slippage_mult: float = 1.0, fallback_cost_r: float = 0.04) -> tuple[float, float, str]:
    if not np.isfinite(risk) or risk <= 0: return fallback_cost_r, 0.0, "fallback"
    prof = symbol_profile(symbol); spread_points, source = _spread_points_at(df, entry_idx, prof); point_size = float(prof.get("point_size", 0.00001)); slippage_points = float(prof.get("default_slippage_points", 2)) * slippage_mult; commission_r = float(prof.get("commission_r", 0.0)); default_cost_r = float(prof.get("default_cost_r", fallback_cost_r))
    computed = (spread_points * point_size * spread_mult + slippage_points * point_size) / risk + commission_r
    if source == "profile_default": computed = max(default_cost_r, computed)
    return round(float(max(0.0, computed)), 6), round(float(spread_points * spread_mult), 3), source


def backtest(df, buy, sell, rr, slm, horizon, cost: float | None = None, symbol: str = "", spread_mult: float = 1.0, slippage_mult: float = 1.0):
    trades = []; i = 250; n = len(df); fallback_cost = 0.04 if cost is None else float(cost)
    while i < n - 2:
        ...
        risk = a * slm; sl = entry - side * risk; tp = entry + side * risk * rr; end = min(ei + horizon, n - 1); R = None; exit_reason = "timeout"; exit_price = float(df.close.iloc[end])
        cost_r, spread_points_used, cost_source = estimate_cost_r(df, ei, symbol, risk, spread_mult=spread_mult, slippage_mult=slippage_mult, fallback_cost_r=fallback_cost)
        for j in range(ei, end + 1):
            hi, lo = float(df.high.iloc[j]), float(df.low.iloc[j]); hit_sl = lo <= sl if side == 1 else hi >= sl; hit_tp = hi >= tp if side == 1 else lo <= tp
            if hit_sl and hit_tp: R = -1 - cost_r; end = j; exit_reason = "ambiguous_sl_first"; exit_price = sl; break
            if hit_sl: R = -1 - cost_r; end = j; exit_reason = "sl"; exit_price = sl; break
            if hit_tp: R = rr - cost_r; end = j; exit_reason = "tp"; exit_price = tp; break
        if R is None: exit_price = float(df.close.iloc[end]); R = side * (exit_price - entry) / risk - cost_r
        entry_time = pd.to_datetime(df.time.iloc[ei]); exit_time = pd.to_datetime(df.time.iloc[end])
        trades.append({"signal_index": int(i), "entry_index": int(ei), "exit_index": int(end), "entry_time": str(entry_time), "exit_time": str(exit_time), "side": "long" if side == 1 else "short", "entry_price": round(entry, 8), "exit_price": round(float(exit_price), 8), "sl": round(float(sl), 8), "tp": round(float(tp), 8), "atr14": round(float(a), 8), "risk_price": round(float(risk), 8), "rr": float(rr), "sl_mult": float(slm), "horizon_bars": int(horizon), "bars_held": int(end - ei), "exit_reason": exit_reason, "spread_points_used": spread_points_used, "cost_r": cost_r, "cost_source": cost_source, "R": float(round(R, 6)), "year": int(entry_time.year), "month": int(entry_time.month)})
        i = end + 1
...
def st(tr):
    ...
    for x in R: cur = cur + 1 if x <= 0 else 0; streak = max(streak, cur)
...
def concepts_for_tf(tf):
    core = ["breakout_trend", "breakout_fast", "pullback_ema21", "compression_breakout", "sweep_reclaim", "prev_day_sweep"]; smc = ["equal_high_low_sweep", "bos_breakout", "choch_reversal", "fvg_rebalance", "order_block_retest"]; intraday = ["asian_breakout"] if tf in ["M5", "M15", "M30", "H1"] else []
...
def discover_edges(name, mode, symbols, tfs, logger):
    ...
    out = OUTPUTS_DIR / name; out.mkdir(parents=True, exist_ok=True); results = []; datasets = []; logger(f"Auto discovery started. mode={mode}, datasets={len(cat)}"); broker = load_broker_profile()
    for _, r in cat.iterrows():
        sym, tf = r.symbol, r.tf; fp = feature_path(sym, tf)
        if not fp.exists(): continue
        df = pd.read_pickle(fp); datasets.append({"symbol": sym, "tf": tf, "rows": len(df), "start": str(df.time.min()), "end": str(df.time.max()), "has_spread_points": "spread_points" in df.columns}); logger(f"Scanning {sym} {tf}: rows={len(df)}")
                ...
                for sess in sessions:
                    ...
                    for rr in [1.0, 1.4, 2.0, 2.5]:
                        for slm in [1.0, 1.4, 2.0]:
                            tr = backtest(df, b0 & sm, s0 & sm, rr, slm, HORIZON.get(tf, 48), symbol=sym); base = st(tr)
                            if not base or base["n"] < min_tr: continue
                            split = max(1, int(len(tr) * 0.7)); test = st(tr.iloc[split:]) or {}; mon = tr.groupby(["year", "month"]).R.sum(); pos = float((mon > 0).mean()) if len(mon) else 0
                            ...
                            rec["setup_id"] = row_setup_id(rec); score = rec["expR"] * 100 + min(rec["pf"], 3) * 20 + (min(float(rec["test_pf"]), 3) * 12 if rec["test_pf"] != "" else 0) + pos * 20 - rec["maxDD_R"] * 0.35 - rec["max_loss_streak"] * 1.5
                            reasons = []
    ...
    res = pd.DataFrame(results).sort_values("score", ascending=False) if results else pd.DataFrame(); cand = res[res.status == "candidate"] if not res.empty else pd.DataFrame(); rej = res[res.status == "rejected"] if not res.empty else pd.DataFrame(); cards = [edge_card(x) for x in cand.head(25).to_dict("records")] if not cand.empty else []
...
def edge_card(r):
    return {"id": r.get("setup_id") or row_setup_id(r), "title": f"{r['symbol']} {r['tf']} {r['concept'].replace('_', ' ').title()}", "symbol": r["symbol"], "tf": r["tf"], "concept": r["concept"], "setup_id": r.get("setup_id") or row_setup_id(r), "grade": r["grade"], "status": r["status"], "score": r["score"], "metrics": {"trades": r["n"], "profit_factor": r["pf"], "test_pf": r.get("test_pf", ""), "expectancy_R": r["expR"], "max_dd_R": r["maxDD_R"], "winrate": r["winrate"], "positive_month_pct": r["positive_month_pct"], "loss_streak": r["max_loss_streak"], "avg_cost_R": r.get("avg_cost_R", "")}, "setup": {"session": r["session"], "lookback": r["lookback"], "rr": r["rr"], "sl_atr": r["sl_mult"]}, "verdict": r["verdict"], "next_step": "Run deeper walk-forward + spread/slippage stress before EA export."}
...
def make_report(name, datasets, res, cand, rej, cards):
    ...
    if cards:
        ...
        for c in cards[:10]: lines.append(f"### {c['title']} — Grade {c['grade']}\n- Setup ID: {c['setup_id']}\n- PF: {c['metrics']['profit_factor']}\n- Test PF: {c['metrics']['test_pf']}\n- Trades: {c['metrics']['trades']}\n- Max DD: {c['metrics']['max_dd_R']}R\n- Avg cost: {c['metrics'].get('avg_cost_R', '')}R\n- Verdict: {c['verdict']}\n\n")
    cols = ["setup_id", "symbol", "tf", "concept", "session", "lookback", "rr", "sl_mult", "grade", "score", "n", "pf", "test_pf", "expR", "maxDD_R", "winrate", "positive_month_pct", "avg_cost_R", "verdict"]
...
def list_outputs():
    ...
    for d in sorted([p for p in OUTPUTS_DIR.iterdir() if p.is_dir()], reverse=True):
        cand = d / "candidate_edges.csv"; allp = d / "all_edges.csv"
        out.append({"name": d.name, "candidate_count": len(pd.read_csv(cand)) if cand.exists() else 0, "all_count": len(pd.read_csv(allp)) if allp.exists() else 0, "has_report": (d / "QUANTLAB_REPORT.md").exists(), "has_sensitivity": (d / "stage6_sensitivity.csv").exists(), "has_portfolio_risk": (d / "stage7_portfolio_risk.csv").exists()})
    return out


def read_edges_preview(scan_name, kind="candidate", limit=100):
    fn = {"candidate": "candidate_edges.csv", "all": "all_edges.csv", "rejected": "rejected_edges.csv", "validation": "stage2_validation.csv", "walkforward": "stage3_walkforward.csv", "stress": "stage4_execution_stress.csv", "monte_carlo": "stage5_monte_carlo.csv", "sensitivity": "stage6_sensitivity.csv", "portfolio": "stage7_portfolio_risk.csv"}.get(kind, "candidate_edges.csv")
...
def clean_outputs():
    ...
    for p in OUTPUTS_DIR.iterdir() if OUTPUTS_DIR.exists() else []:
        ...
        elif p.name == "latest_edge_cards.json": p.unlink(missing_ok=True); removed += 1
