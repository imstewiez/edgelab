...
export function App() {
  ...
  return <div className="app"><aside className="sidebar"><div className="brand"><div className="logo"><FlaskConical size={24}/></div><div><strong>CoreEA EdgeLab</strong><span>Quant lab</span></div></div>{[['run', Zap, 'Run'], ['edges', CheckCircle2, 'Edges'], ['universe', Layers3, 'Universe'], ['data', Database, 'Data'], ['logs', Activity, 'Logs']].map(([id, Icon, txt]: any) => <button key={id} className={tab === id ? 'nav active' : 'nav'} onClick={() => setTab(id)}><Icon size={18}/>{txt}</button>)}<div className="sideFooter"><ShieldCheck size={16}/><span>Local data. No paid DB.</span></div></aside>
    <main className="main"><header className="topbar"><div><h1>{pageTitle(tab)}</h1><p>{pageSub(tab)}</p></div><Button onClick={refreshAll}>Refresh</Button></header><div className={`status ${activeJob ? 'running' : ''}`}><strong>{activeJob ? `${label(activeJob.kind)} running` : 'Status'}</strong><span>{activeJob ? (activeJob.logs?.slice(-1)[0] || activeJob.status) : notice}</span></div>
      {tab === 'run' && <section className="grid"><Stat label="Engine" value={health?.ok ? 'Online' : 'Offline'} icon={<Activity/>} tone={health?.ok ? 'good' : 'bad'} /><Stat label="Candidates" value={latestRun?.candidate_count ?? 0} icon={<CheckCircle2/>} tone="good" /><Stat label="WF Pass" value={walkforward?.wf_pass ?? 0} icon={<ShieldCheck/>} tone="good" /><Stat label="EA Ready" value={walkforward?.ea_ready ?? 0} icon={<ShieldCheck/>} tone="warn" />
        <Card className="wide primaryPanel"><div><h2>Discovery → Validation → Walk-forward</h2><p>Discovery screens ideas. Validation stress-checks them. Walk-forward checks whether they survive different time windows. EA-ready remains 0 until full execution realism exists.</p></div><div><Button className="primary huge" busy={busy === 'Discover Edges'} disabled={!!activeJob} onClick={() => run('Discover Edges', () => api.discover({ mode: 'auto' }), 'logs')}>Discover Edges</Button><Button disabled={!!activeJob || !latestRun} onClick={() => run('Validate', () => api.validate({ scan_name: selectedOutput || latestRun?.name }), 'logs')}>Validate</Button><Button disabled={!!activeJob || !latestRun} onClick={() => run('Walk-forward', () => api.runWalkforward({ scan_name: selectedOutput || latestRun?.name }), 'logs')}>Walk-forward</Button></div></Card>
        <Card className="wide"><h2>Pipeline status</h2>{latestRun ? <div className="summaryGrid"><div><strong>{latestRun.all_count}</strong><span>Screened</span></div><div><strong>{latestRun.candidate_count}</strong><span>Candidates</span></div><div><strong>{validation?.robust_candidates || 0}</strong><span>Robust</span></div><div><strong>{walkforward?.wf_pass || 0}</strong><span>WF pass</span></div></div> : <div className="empty">No run yet. Click Discover Edges.</div>}</Card>
        <Card className="wide"><h2>Walk-forward leaders</h2><WalkForwardList rows={walkforward?.top || []} /></Card><Card className="wide"><h2>Stage 2 robust candidates</h2><ValidationList rows={validation?.top || []} /></Card></section>}
      {tab === 'edges' && <section className="grid"><Card className="wide"><h2>Choose run</h2><select value={selectedOutput} onChange={e => setSelectedOutput(e.target.value)}><option value="">Select run</option>{outputs.map(o => <option key={o.name} value={o.name}>{o.name} — {o.candidate_count} candidates / {o.all_count} screened</option>)}</select><Button disabled={!!activeJob || !selectedOutput} onClick={() => run('Validate', () => api.validate({ scan_name: selectedOutput }), 'logs')}>Validate This Run</Button><Button disabled={!!activeJob || !selectedOutput} onClick={() => run('Walk-forward', () => api.runWalkforward({ scan_name: selectedOutput }), 'logs')}>Walk-forward This Run</Button></Card><Card className="wide"><h2>Stage 3 walk-forward</h2><div className="summaryGrid"><div><strong>{walkforward?.candidates_checked || 0}</strong><span>Checked</span></div><div><strong>{walkforward?.wf_pass || 0}</strong><span>Pass</span></div><div><strong>{walkforward?.wf_watchlist || 0}</strong><span>Watchlist</span></div><div><strong>{walkforward?.ea_ready || 0}</strong><span>EA-ready</span></div></div><p>{walkforward?.warning}</p><WalkForwardList rows={walkforward?.top || []} /></Card><Card className="wide"><h2>Stage 2 robustness</h2><ValidationList rows={validation?.top || []} /></Card><Card className="wide"><h2>Accepted discovery candidates</h2><EdgeList cards={bestCards} /></Card><Card className="wide"><h2>Rejected ideas</h2><MiniTable rows={rejectedRows} columns={['symbol','tf','concept','pf','test_pf','expR','maxDD_R','positive_month_pct','verdict']} /></Card></section>}
