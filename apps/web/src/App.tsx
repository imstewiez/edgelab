...
export function App() {
  ...
  const [walkforward, setWalkforward] = useState<any>({ wf_pass: 0, wf_watchlist: 0, wf_fail: 0, ea_ready: 0, top: [] })
  const [stress, setStress] = useState<any>({ stress_pass: 0, stress_watchlist: 0, stress_fail: 0, ea_ready: 0, top: [] })
  ...
  async function refreshAll() {
    ...
    try { setWalkforward(await api.walkforward()) } catch {}
    try { setStress(await api.executionStress()) } catch {}
  }
  ...
  useEffect(() => {
    ...
    api.walkforward(selectedOutput).then(setWalkforward).catch(() => {})
    api.executionStress(selectedOutput).then(setStress).catch(() => {})
  }, [selectedOutput])
  ...
  return <div className="app"><aside className="sidebar"><div className="brand"><div className="logo"><FlaskConical size={24}/></div><div><strong>CoreEA EdgeLab</strong><span>Quant lab</span></div></div>{[['run', Zap, 'Run'], ['edges', CheckCircle2, 'Edges'], ['universe', Layers3, 'Universe'], ['data', Database, 'Data'], ['logs', Activity, 'Logs']].map(([id, Icon, txt]: any) => <button key={id} className={tab === id ? 'nav active' : 'nav'} onClick={() => setTab(id)}><Icon size={18}/>{txt}</button>)}<div className="sideFooter"><ShieldCheck size={16}/><span>Local data. No paid DB.</span></div></aside>
    <main className="main"><header className="topbar"><div><h1>{pageTitle(tab)}</h1><p>{pageSub(tab)}</p></div><Button onClick={refreshAll}>Refresh</Button></header><div className={`status ${activeJob ? 'running' : ''}`}><strong>{activeJob ? `${label(activeJob.kind)} running` : 'Status'}</strong><span>{activeJob ? (activeJob.logs?.slice(-1)[0] || activeJob.status) : notice}</span></div>
      {tab === 'run' && <section className="grid"><Stat label="Engine" value={health?.ok ? 'Online' : 'Offline'} icon={<Activity/>} tone={health?.ok ? 'good' : 'bad'} /><Stat label="Candidates" value={latestRun?.candidate_count ?? 0} icon={<CheckCircle2/>} tone="good" /><Stat label="Stress Pass" value={stress?.stress_pass ?? 0} icon={<ShieldCheck/>} tone="good" /><Stat label="EA Ready" value={stress?.ea_ready ?? 0} icon={<ShieldCheck/>} tone="warn" />
        <Card className="wide primaryPanel"><div><h2>Discovery → Validation → Walk-forward → Execution stress</h2><p>Stage 4 checks whether ideas survive worse spread, slippage, delayed fills and bad execution assumptions. EA-ready remains 0 until Monte Carlo + forward tracking.</p></div><div><Button className="primary huge" busy={busy === 'Discover Edges'} disabled={!!activeJob} onClick={() => run('Discover Edges', () => api.discover({ mode: 'auto' }), 'logs')}>Discover Edges</Button><Button disabled={!!activeJob || !latestRun} onClick={() => run('Validate', () => api.validate({ scan_name: currentRunName }), 'logs')}>Validate</Button><Button disabled={!!activeJob || !latestRun} onClick={() => run('Walk-forward', () => api.runWalkforward({ scan_name: currentRunName }), 'logs')}>Walk-forward</Button><Button disabled={!!activeJob || !latestRun} onClick={() => run('Execution Stress', () => api.runExecutionStress({ scan_name: currentRunName }), 'logs')}>Execution Stress</Button></div></Card>
        <Card className="wide"><h2>Pipeline status</h2>{latestRun ? <div className="summaryGrid"><div><strong>{latestRun.all_count}</strong><span>Screened</span></div><div><strong>{validation?.robust_candidates || 0}</strong><span>Robust</span></div><div><strong>{walkforward?.wf_pass || 0}</strong><span>WF pass</span></div><div><strong>{stress?.stress_pass || 0}</strong><span>Stress pass</span></div></div> : <div className="empty">No run yet. Click Discover Edges.</div>}</Card>
        <Card className="wide"><h2>Execution-stress leaders</h2><StressList rows={stress?.top || []} /></Card><Card className="wide"><h2>Walk-forward leaders</h2><WalkForwardList rows={walkforward?.top || []} /></Card></section>}
      {tab === 'edges' && <section className="grid"><Card className="wide"><h2>Choose run</h2><select value={selectedOutput} onChange={e => setSelectedOutput(e.target.value)}><option value="">Select run</option>{outputs.map(o => <option key={o.name} value={o.name}>{o.name} — {o.candidate_count} candidates / {o.all_count} screened</option>)}</select><Button disabled={!!activeJob || !selectedOutput} onClick={() => run('Validate', () => api.validate({ scan_name: selectedOutput }), 'logs')}>Validate This Run</Button><Button disabled={!!activeJob || !selectedOutput} onClick={() => run('Walk-forward', () => api.runWalkforward({ scan_name: selectedOutput }), 'logs')}>Walk-forward This Run</Button><Button disabled={!!activeJob || !selectedOutput} onClick={() => run('Execution Stress', () => api.runExecutionStress({ scan_name: selectedOutput }), 'logs')}>Stress This Run</Button></Card><Card className="wide"><h2>Stage 4 execution stress</h2><div className="summaryGrid"><div><strong>{stress?.candidates_checked || 0}</strong><span>Checked</span></div><div><strong>{stress?.stress_pass || 0}</strong><span>Pass</span></div><div><strong>{stress?.stress_watchlist || 0}</strong><span>Watchlist</span></div><div><strong>{stress?.ea_ready || 0}</strong><span>EA-ready</span></div></div><p>{stress?.warning}</p><StressList rows={stress?.top || []} /></Card><Card className="wide"><h2>Stage 3 walk-forward</h2><WalkForwardList rows={walkforward?.top || []} /></Card><Card className="wide"><h2>Stage 2 robustness</h2><ValidationList rows={validation?.top || []} /></Card><Card className="wide"><h2>Rejected ideas</h2><MiniTable rows={rejectedRows} columns={['symbol','tf','concept','pf','test_pf','expR','maxDD_R','positive_month_pct','verdict']} /></Card></section>}
...
function StressList({ rows }: { rows: any[] }) { if (!rows?.length) return <div className="empty">No Stage 4 execution stress yet. Run Execution Stress after walk-forward.</div>; return <MiniTable rows={rows} columns={['symbol','tf','concept','stress_status','stress_pass_rate','base_pf','base_test_pf','worst_test_pf','worst_maxDD_R','verdict']} /> }
function WalkForwardList({ rows }: { rows: any[] }) { if (!rows?.length) return <div className="empty">No Stage 3 walk-forward yet. Run Walk-forward after discovery.</div>; return <MiniTable rows={rows} columns={['symbol','tf','concept','wf_status','wf_score','wf_windows','wf_passed_windows','wf_pass_rate','wf_median_pf','wf_min_pf','wf_median_expR','wf_verdict']} /> }
function ValidationList({ rows }: { rows: any[] }) { if (!rows?.length) return <div className="empty">No Stage 2 validation yet. Run Validate after discovery.</div>; return <MiniTable rows={rows} columns={['symbol','tf','concept','robustness_status','robustness_score','trades','pf','test_pf','stress_test_pf','stress_maxDD_R','verdict']} /> }
...
function pageSub(tab: Tab) { return { run: 'Discovery + validation + walk-forward + execution stress.', edges: 'Candidate ideas through robustness, walk-forward and execution-stress gates.', universe: 'All concepts EdgeLab knows about, including SMC and DOM roadmap.', data: 'Upload files and check whether the data is usable.', logs: 'Only for debugging when something goes wrong.' }[tab] }
