  ...
  return <div className="app">
    ...
    <main className="main">
      ...
      {tab === 'run' && <section className="grid">
        ...
        <Stat label="Screened Ideas" value={latestRun?.all_count ?? 0} icon={<FlaskConical/>} />
        <Card className="wide primaryPanel"><div><h2>Find market edges automatically</h2><p>Tests classic, breakout, trend, pullback and SMC-style hypotheses already implemented. DOM/tick concepts are mapped but require extra data.</p></div><Button className="primary huge" busy={busy === 'Discover Edges'} disabled={!!activeJob} onClick={() => run('Discover Edges', () => api.discover({ mode: 'auto' }), 'logs')}>Discover Edges</Button></Card>
        <Card className="wide"><h2>What happened last run?</h2>{latestRun ? <div className="summaryGrid"><div><strong>{latestRun.candidate_count}</strong><span>First-pass candidates</span></div><div><strong>{latestRun.all_count}</strong><span>Screened ideas</span></div><div><strong>{rejectedRows.length}</strong><span>Rejected / weak</span></div><div><strong>{selectedOutput || latestRun.name}</strong><span>Run ID</span></div></div> : <div className="empty">No run yet. Click Discover Edges.</div>}</Card>
        <Card className="wide"><h2>Best edges found</h2><EdgeList cards={bestCards.slice(0, 5)} /></Card>
      </section>}

      {tab === 'edges' && <section className="grid">
        <Card className="wide"><h2>Choose run</h2><select value={selectedOutput} onChange={e => setSelectedOutput(e.target.value)}><option value="">Select run</option>{outputs.map(o => <option key={o.name} value={o.name}>{o.name} — {o.candidate_count} candidates / {o.all_count} screened</option>)}</select></Card>
      ...
      {tab === 'universe' && <section className="grid">
        <Stat label="Concept Groups" value={universe.summary?.groups || 0} icon={<Layers3/>} />
        <Stat label="Total Concepts" value={universe.summary?.concepts || 0} icon={<FlaskConical/>} />
        <Stat label="Active Now" value={universe.summary?.active || 0} icon={<CheckCircle2/>} tone="good" />
        <Stat label="Need Tick/DOM" value={universe.summary?.requires_tick_or_dom || 0} icon={<AlertTriangle/>} tone="warn" />
...
function pageSub(tab: Tab) { return { run: 'Start the full research pipeline and see the answer, not raw tables.', edges: 'Accepted and rejected ideas in plain language.', universe: 'All concepts EdgeLab knows about, including SMC and DOM roadmap.', data: 'Upload files and check whether the data is usable.', logs: 'Only for debugging when something goes wrong.' }[tab] }
