...
export const api = {
  ...
  runSensitivity: (payload: any = {}) => req('/api/jobs/sensitivity', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload) }),
  portfolioRisk: (scan?: string) => req(`/api/portfolio-risk${scan ? `?scan_name=${encodeURIComponent(scan)}` : ''}`),
  runPortfolioRisk: (payload: any = {}) => req('/api/jobs/portfolio-risk', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(payload) }),
  jobs: () => req('/api/jobs'),
